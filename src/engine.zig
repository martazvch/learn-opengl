...
pub fn setupWindow() void {
    ...
    while (glfw.glfwWindowShouldClose(window) == 0) {
        ...
        var trans = math.Mat4.identity;
        trans = trans.mul(.createAngleAxis(math.vec3(0, 0, 1), time));
        trans = trans.mul(.createTranslationXYZ(0.5, -0.5, 0.0));
        shader_prog.setMat4f("transform", trans);
